...
class LoraDiffusion(torch.nn.Module):
    ...
    def __init__(self, data_dim=1_365_504, 
        ...
        self.ups = nn.ModuleList([ResnetBlock(num_layers=layers_per_block, cond_dim=cond_dim,  in_dim=model_dim*2, mid_dim=int((model_dim) * ff_mult), ada_dim=model_dim//2) for _ in range(num_blocks)])
