...
class LoraDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # z = mean + eps * std
        return z.float(), face_embedding[0].flatten().float(), face_embedding[1].flatten().float(), file
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, (batch, clip_embs, face_embeddings, files) in enumerate(train_dataloader):
            with accelerator.accumulate(lora_diffusion):
                ...
                pred, pred_cond = lora_diffusion(noisy_model_input, timesteps, _face_embedding)
                ...
                total_clip_loss = F.mse_loss(pred_cond, _face_embedding_norm, reduction="mean") #* 0.0
                # pred_cond = True
                # if  is not None:
                #     pass
                #     # pred_cond = pred_cond / pred_cond.norm(dim=-1, keepdim=True)
