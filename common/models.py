...
class Norm(nn.Module):
    """
    like an adanorm layer with extra flexibility
    """

    def __init__(self, in_dim, 
                        norm_type="layer", # ["none", "layer", "rms"],
                        shift=True, 
                        scale=True,
                        ):
        super().__init__()
        self.scale = scale_op if scale else identity
        self.shift = add if shift else identity
        self.scale_weight = nn.Parameter(torch.zeros(in_dim)) if scale else None
        self.shift_weight = nn.Parameter(torch.zeros(in_dim)) if shift else None
        self.set_norm(norm_type, in_dim)

    
    def set_norm(self, norm_type, in_dim):
        if norm_type == "rms":
            self.norm = RMSNorm()
            # only scale
            self.shift = identity
    ...
    def forward(self, x: torch.Tensor, dummy=None) -> torch.Tensor:
        x = self.norm(x)
        x = self.scale(x, self.scale_weight)
        x = self.shift(x, self.shift_weight)
        return x
