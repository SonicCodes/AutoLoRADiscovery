...
class MyDataset(Dataset):
    ...
    def __init__(
        ...
        size=512,
        center_crop=False,
        num_dataloader_repeats=50,
    ):
        self.size = size
        self.center_crop = center_crop
        self.tokenizer = tokenizer
        # square_images = torch.load("/home/ubuntu/AutoLoRADiscovery/square_images.pth")
        self.instance_images_path = [os.path.join(instance_data_root, f) for f in os.listdir(instance_data_root) if f.endswith(".jpg") or f.endswith(".png") or f.endswith(".jpeg")]
        random.shuffle(self.instance_images_path)
        self.instance_images_path = self.instance_images_path[:50]
    ...
    for tracker in accelerator.trackers:
        if args.use_wandb:
            ...
            if tracker.name == "wandb":
                # canvas.save(temporary_file_path, format="JPEG", quality=20)
                tracker.log(
                    {
                        phase_name: [
                            wandb.Image(gtmage(image), caption=f"{i}: {args.validation_prompt[i]}") for i, image in enumerate(images)
                        ]
                    }
                )
...
def more_init(accelerator, args, train_dataloader, train_dataset, logger, num_update_steps_per_epoch, global_step, wandb_name="diffusion_lora", wandb_runname=None):
    if accelerator.is_main_process:
        tracker_config = vars(copy.deepcopy(args))
        if args.use_wandb:
            accelerator.init_trackers(wandb_name, config=tracker_config, init_kwargs={"wandb":{"name":wandb_runname}})
