...
def train(args):
    ...
    global_step, first_epoch, progress_bar = more_init(accelerator, args, train_dataloader, 
                                                        train_dataset, logger, num_update_steps_per_epoch, global_step, wandb_name="train_lora", wandb_runname=args.celeb_name)
...
if __name__ == "__main__":
    # if there are command line arguments like "--celeb_name", "--instance_data_dir", "--output_dir", apply them to the default_arguments
    # and call train with the modified arguments
    if len(sys.argv) > 1:
        for i in range(1, len(sys.argv), 2):
            default_arguments[sys.argv[i].replace("--", "")] = sys.argv[i+1]
