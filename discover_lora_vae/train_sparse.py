...
default_arguments = dict(
    ...
    output_dir="model-output-sp",
    seed=None,
    train_batch_size=128,
    max_train_steps=30_000,
    num_dataloader_repeats=100,
    checkpointing_steps=500,
    # resume_from_checkpoint="/home/ubuntu/AutoLoRADiscovery/discover_lora_vae/model-output-sp/checkpoint-sph-15500",
...
def log_activations(activations, step):
    # Ensure activations are a NumPy array
    if isinstance(activations, torch.Tensor):
        activations = activations.detach().cpu().numpy()
    
    # Reduce the batch dimension by taking the maximum value over the batch
    max_activations = np.max(activations, axis=0)
    
    # Log histogram of max activations
    wandb.log({"activations_histogram": wandb.Histogram(max_activations)}, step=step)
    
    # Log max activations as a line plot
    wandb.log({"max_activations": wandb.plot.line_series(
        xs=list(range(len(max_activations))),
        ys=[max_activations],  # Reshape to 2D array
        keys=["Max Activations"],
...
def train(args):
    ...
    lora_vae.initiate_vae(
        "/home/ubuntu/AutoLoRADiscovery/discover_lora_vae/checkpoint-14000",
    ...
    def train_step(first=0.4, first_with_orig=0.01, second=0.4, second_w_orig=0.01, noise_std=0.01):
        ...
        recon_x, ground_truth, sparse_logits, sparsity_loss, corr_loss = lora_vae(batch) 
        mse_loss = F.mse_loss(recon_x.float(), ground_truth.float(), reduction="mean")


        loss = (mse_loss) + sparsity_loss + corr_loss
        ...
        return mse_loss, grad_norm, sparse_logits, sparsity_loss, corr_loss


    grad_norm = 0
    for i in range(args.max_train_steps):
        with accelerator.accumulate(lora_vae):
            mse_loss, grad_norm, sparse_logits, sp_loss, corr_loss = train_step() 
        ...
        if accelerator.sync_gradients:
            ...
            if accelerator.is_main_process:
                if global_step % args.checkpointing_steps == 0:
                    save_path = os.path.join(args.output_dir, f"checkpoint-sph-{global_step}")
                    torch.save(unwrap_model(accelerator, lora_vae).state_dict(), save_path)
        
        log_activations(sparse_logits, global_step)
        count_non_zero = ((1-(torch.count_nonzero(sparse_logits) / (sparse_logits.size(0) * sparse_logits.size(1)))) * 100).detach().item()
        max_val = sparse_logits.max().detach().item()
        min_val = sparse_logits.min().detach().item()
        ...
        "max": max_val,
        "min": min_val,
        "corr_loss": corr_loss.detach().item()
         }
